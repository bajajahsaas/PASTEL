...
modelName=simpleModelGlove2BothPretrainedPointer0_10

for sty in STYLED #ethnic gender Country edu TOD #Politics
do
    echo "----------------------"
    echo "Training Model For $sty"
    CUDA_VISIBLE_DEVICES="1" python -u main.py -mode=train -preTrain -pointer -NUM_PRETRAIN_EPOCHS=0 -initGlove2 -initGloveEncode2 -modelName=${modelName}_$sty -emb_size=300 -hidden_size=384  -problem=$sty -NUM_EPOCHS=10 | tee logsmodel/${modelName}_$sty
    echo "Finished Training Model for $sty"
    echo "----------xxx------------------"
done
