...
do
    echo "----------------------"
    echo "Training Model For $sty"
    CUDA_VISIBLE_DEVICES="1" python -u main.py -mode=train -preTrain -pointer -NUM_PRETRAIN_EPOCHS=20 -initGlove2 -initGloveEncode2 -modelName=${modelName}_$sty -emb_size=300 -hidden_size=384  -problem=$sty -NUM_EPOCHS=20 | tee logsmodel/${modelName}_$sty
    echo "Finished Training Model for $sty"
    echo "----------xxx------------------"
done
