...
class Lang():
    ...
    def fuseVocab(self,externalLang):
       
        for word,wordIndex in externalLang.wids.items():
            if word not in self.wids:
                self.wids[word]=len(self.wids)
        
        for word,wordIndex in self.wids.items():
            if word not in externalLang.wids:
                externalLang.wids[word]=len(externalLang.wids)
