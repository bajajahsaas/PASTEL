...
class EncoderBiRNNModel(nn.Module):
    ...
    def __init__(self, wids, vocabSize, emb_size, hidden_size, use_LSTM, share_embeddings, reference_embeddings=None):
        ...
        self.encoder = EncoderRNN(wids, vocabSize, emb_size, hidden_size, use_LSTM, share_embeddings,
                                  reference_embeddings)
        if not share_embeddings:
            reference_embeddings = self.encoder.embeddings
        self.revcoder = EncoderRNN(wids, vocabSize, emb_size, hidden_size, use_LSTM, share_embeddings=True,
    ...
    def forward(self, srcBatch, srcMask, hidden, srcMasking=False, inference=False):
        ...
        for rowId, row in enumerate(srcBatch):  ## seqlen times. row: b
            ...
            enc_out, enc_hidden = self.encoder(srcBatch.shape[1], srcEmbedIndex,
                                               enc_hidden)  # encoder( value b, (b,embedding_size), b,enc_hidden_size ) .  return is enc_out,enc_hidden.
...
class AttnDecoderRNN(nn.Module):
    def __init__(self, wids, vocabSize, emb_size, hidden_size, use_LSTM, use_attention, share_embeddings, pointer,
                 decAttn, reference_embeddings=None, sigmoid=False):
...
class EncoderRNNMultiRep(nn.Module):
    def __init__(self, wids, vocab_sizes, emb_sizes, hidden_size, use_LSTM, share_embeddings,
    ...
    def forward(self, batchSize, embed_indices, hidden):
        ...
        out, hidden = self.encoder(embeds.view(1, batchSize, -1),
                                   hidden)  # ret is batch_size x element_size x embedding_size
        return out, hidden
...
class AttnDecoderRNNMutipleInput(nn.Module):
    def __init__(self, wids, vocabSize, emb_size, hidden_size, use_LSTM, use_attention, share_embeddings,
                 reference_embeddings=None, extra_dims=0, typ="entire_as_tuple"):
    ...
    def forward(self, batchSize, tgtEmbedIndex, lst_of_encoderOutTensor, o_t, hidden, feedContextVector=False,
                contextVector=None, inference=False):
...
class AttnDecoderRNNMutipleInputCNN(nn.Module):
    def __init__(self, wids, vocabSize, emb_size, hidden_size, use_LSTM, use_attention, share_embeddings,
                 reference_embeddings=None, extra_dims=0, typ="entire_as_tuple", cnn_output_size=-1):
...
class CNN(nn.Module):
    def __init__(self, inp_channels=1, output_channels=16, kernel_size=2, padding=1):
        ...
        self.layer1 = nn.Conv2d(inp_channels, output_channels / 2, kernel_size=kernel_size,
                                padding=padding)  # input channels, output channels, kernel_size, padding
